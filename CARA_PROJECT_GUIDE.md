...
**Resolved (Session 6, June 20, 2026):** `doPost` previously used `mode: 'no-cors'`, which meant the app fired the request and immediately showed success regardless of whether the write actually reached the Sheet. This caused a real incident — see Session Log — where a staff member's MK check-in appeared done in the app but never landed in the Sheet, producing a false coverage-gap flag in the 6/20 morning report. **Fix:** the app now awaits the real JSON response from `doPost`, retries up to 2 additional times on failure, and — only if all retries are exhausted — shows a small dismissible banner on the dashboard so staff get an honest signal in real time instead of finding out the next morning. The celebration screen still shows immediately either way; this only adds a safety net, it doesn't slow down the check-in flow. `doPost` also now logs resident/staff/outcome on every call (it previously had zero logging), so a future incident like this can be diagnosed directly from the Executions panel instead of being forensically unrecoverable. **Not yet live-tested against the deployed Apps Script endpoint, and `Code.gs` needs to be redeployed** — Claude cannot reach script.google.com from its sandbox, so confirm with a real check-in before trusting it for the next shift.
...
| Check-in logging confirmation | Switched from `no-cors` fire-and-forget to awaited fetch + JSON response check + retry (3 attempts, backoff) + dashboard banner on persistent failure | `no-cors` made backend write failures invisible to both app and staff; root cause of a false MK coverage-gap flag in the 6/20 report — the check-in showed as done in-app but the Sheet write silently failed |
...
**Still pending:**
- **Live test required before next shift** — Claude cannot reach `script.google.com` from its sandbox to test the deployed endpoint directly. Do a real check-in against the live `LOG_ENDPOINT` and confirm: (1) successful check-ins still log normally, (2) a deliberately broken endpoint triggers the banner correctly.
- **Redeploy Code.gs** (paste → Save → Deploy → Manage deployments → New version → Deploy) to pick up the new logging — current live deployment has none of it yet, so the 3:48:47 AM / 5:23:08 AM executions from 6/20 remain unidentifiable; this only helps going forward
