...
`doPost` also now logs resident/staff/outcome on every call (it previously had zero logging), so a future incident can be diagnosed directly from the Executions panel. **Live-tested in airplane mode by Sam — confirmed the banner appears and dismisses correctly.** The background-retry and persistent-queue layers (added after that test) have not yet been separately verified — see Session Log for a suggested test.
...
**Still pending:**
- **Redeploy `index.html`** with the two-layer recovery code (background retry + persistent queue) — everything tested live so far was the detection-only version; the recovery layers are new and untested
- **Suggested test for the recovery layers:** airplane mode → complete a check-in → confirm banner appears (as before) → turn airplane mode back **off** → within ~45s (or immediately, since it also listens for the browser's `online` event) the Sheet row should appear and the banner should clear on its own, without reopening the app. Separately: airplane mode → check-in → close the browser tab/app entirely before reconnecting → reopen and log back in with WiFi on → the text-only fields (no photo) should appear in the Sheet automatically; the photo will be missing, which is expected — re-check that resident and retake it.
