...
Last updated: July 16, 2026 (Session 25–26 — YC timed toe photo; urgent photo flag + RN alert email)
...
### July 16, 2026 (Session 26 — urgent photo flag + RN alert email)

**Version: `2026-07-16-b`** (both files).

**What staff see:** a red "🚩 Urgent — RN needs to see this now" checkbox under
every photo zone, including each optional extra. Checking it **blocks submit
until a note is written**, with a prompt appearing beside the comment box
explaining why.

**Design decisions worth preserving:**

- **Per-photo, not per-check-in.** The alert says *"Photo 2 (YC'S TOES) —
  FLAGGED URGENT"* rather than making the nurse guess which of four photos is
  the concern. Each slot carries `urgent` + a human `label` through the payload.
- **A note is mandatory when flagging.** An urgent photo with no explanation
  makes someone phone the house at 3am to ask what they're looking at. Enforced
  in `checkReady()` via `anyUrgent()`; tested against whitespace-only notes.
- **Photos are embedded inline in the email, not just linked.** A link means the
  nurse must be signed into the right Google account on whatever device she's
  holding at 3am. `DriveApp.getFileById().getBlob()` + `inlineImages`, with the
  Drive link kept as a fallback if the blob fetch fails.
- **The alert can never break a check-in.** It fires only AFTER the row is
  written, wrapped in try/catch. A Gmail failure logs `URGENT_ALERT_FAILED`; the
  check-in still succeeds, the flag still lands in `Urgent Flags`, and it still
  surfaces in the next daily report. The clinical record never depends on the
  email working.
- **Fires at submit, not at capture** — so it carries the note and clinical
  context. Accepted trade-off: a check-in flagged and then *abandoned entirely*
  sends no alert (the photo is still safely on Drive). Rare, rarer since the
  Session 23 memory fix.

**⚠️ THE EMAIL SUBJECT IS LOAD-BEARING — DO NOT CHANGE IT:**
```
URGENT — Cara photo flagged for review
```
Stored as `URGENT_SUBJECT` in `Code.gs`. It is **constant** so Leriy's Gmail
filter always matches, and **PHI-free** because subject lines render in
lock-screen previews. Change this string and his phone silently stops buzzing.

**Why Gmail instead of SMS (the notification decision):** researched carrier
email-to-SMS gateways and they are effectively **dead** — AT&T shut down
`txt.att.net` June 17 2025, T-Mobile's `tmomail.net` stopped delivering late
2024 with no announcement, Verizon's `vtext.com` is degraded with a full
shutdown set for March 31 2027. Critically, they fail **silently** — the worst
possible property for an urgent clinical channel. So the real options were
Twilio or email. Sam chose the Gmail route: Leriy filters on the subject above →
applies a label → enables notifications for that label → real push notification,
free, instant, no A2P 10DLC registration.

**Twilio is stubbed, not abandoned — Sam explicitly wants this later.**
`sendUrgentSms()` in `Code.gs` is a deliberate no-op with the exact
`UrlFetchApp` call written out in the comment above it and the PHI-free
`SMS_BODY` already defined. When his campaign clears it's a few lines in one
function, no rework. Two rules recorded there:
- **Credentials go in Script Properties, NEVER in `Code.gs`** — Sam pastes that
  file into working sessions; a token in it would leak into a chat log and let
  someone send messages on ABLE's account.
- **The SMS body must contain NO PHI.** Unencrypted, lands on a lock screen. No
  initials, no photo, no clinical detail. The email carries everything. This is
  also why no BAA with Twilio is needed.

**Twilio setup notes for when Sam is ready:** trial accounts can't register for
A2P 10DLC (must upgrade to paid); campaign review currently takes **10–15 days**;
ABLE has an EIN so it registers as a Business Representative / **Low-Volume
Standard Brand** (<6,000 segments/day). Vague campaign descriptions and unclear
opt-in are the top rejection causes — describe it as internal employee
operational alerts with onboarding consent, not consumer messaging.

**Also added:** `Urgent Flags` column (same append-at-right migration pattern),
and an **URGENT — FLAGGED FOR RN REVIEW** section at the top of the daily report
(red, with the note, confirming an alert was sent) so a flag can't quietly die
in an unopened inbox. Plus a Shift Overview counter.

---

### July 16, 2026 (Session 25 — YC timed toe photo)

**Version: `2026-07-16-a`** (both files).

**Requirement:** a required photo of YC's toes, but only at the **first check-in
after 7:00pm** and the **first check-in after 4:30am** — not every check-in.
Staff note: look for redness, macerated (white) skin, or cracks; if present she
needs Nystatin powder (PRN) 3x/day until resolved.

**Wording unified per Sam:** the "PRN powder" he described IS the Nystatin
already named in YC's first-check task. Both now read **"Nystatin powder (PRN)"**
so staff never see two names for one treatment. Don't reword one without the
other.

**Implementation — `timedPhoto` config on YC:**
```
timedPhoto: { id:'toes', windows:[{h:19,m:0},{h:4,m:30}], label:{...}, note:{...} }
```
Generic enough that another resident's timed photo is config-only, not code.

**The two windows tile the whole day**, so every check-in falls in exactly one:
evening 19:00→04:29, morning 04:30→18:59. `timedPhotoWindowStart()` returns the
most recent boundary that has passed (correctly rolling back to *yesterday's*
19:00 for a 2am check-in). Tested across midnight and the 4:29/4:31 boundary.

**Two decisions that matter:**
- **Verified against the BACKEND, never session memory.** New `Special Photos`
  column records which check-in carried the toe photo; `doGet` returns
  `lastSpecialPhotos` at login; `timedPhotoNeeded()` compares that to the
  current window boundary. Session memory would break the moment staff change,
  a phone reloads, or two caregivers both visit YC at 7:05 and 7:40.
- **Fails OPEN.** If `doGet` doesn't return (offline at login), the app doesn't
  know whether the photo was taken — so it prompts. A redundant toe photo costs
  thirty seconds; a missed one costs a day of untreated maceration.

**Slot arithmetic centralised** in `timedSlotNumber()` / `extraBaseSlot()` so the
photo columns stay correct as required photos come and go. Order is always:
primary → FSC repositioning (if any) → timed (if due) → extras. Verified that
CJ/FSC residents are completely unaffected.

**Still pending:**
- **Leriy: set up the Gmail filter** on `URGENT — Cara photo flagged for review`
  → label → enable notifications for that label. Without this the urgent email
  arrives but his phone doesn't buzz.
- **Test the urgent flag end to end** — flag with no note (submit should stay
  disabled), add note, submit, confirm email arrives with photo visible and
  `Urgent Flags` reads `Photo N (label)`.
- **Test the toe photo** — YC before 7pm (no zone), after 7pm (zone appears,
  blocks submit), second check-in (zone gone). `Special Photos` column should
  read `toes`.
- **Twilio SMS** when Sam is ready — see Session 26 notes above
- Lory M.'s device — check Debug Log for `STALE_VERSION`; get her onto current build
- Leriy to review a real 2400px/0.92 photo — the open compression question
- Confirm Sam is OK with Drive URLs persisting in the localStorage queue (Session 23)
- Watch whether "app kicks me out" reports stop
- Watch reports for the Session 19 gap-calculation fix
- Continue watching MK's room signal as extender position is tuned
- The gentle-lateness-encouragement decision (Session 14, awaiting Sam)
- Native speaker review of ES/HT translations (now includes toe-photo and urgent strings)
